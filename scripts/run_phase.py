...
def main() -> None:
    ...
    if results.exists():
        import csv
        with results.open(encoding="utf-8", newline="") as f: prior = sum(float(r.get("actual_cost_usd") or r.get("estimated_cost_usd") or 0) for r in csv.DictReader(f))
    ...
    for ordinal, run in enumerate(schedule, 1):
        ...
        try:
            ...
            if prior >= float(config["max_api_budget_usd"]):
                metadata["budget_hard_stop"] = True
                write_json(run_dir / "metadata.json", metadata)
                raise SystemExit("Budget hard stop reached after completed run.")
