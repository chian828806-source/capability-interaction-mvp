...
def main() -> None:
    ...
    cmd=[str(Path(sys.executable).parent/"bench"),"eval","run","--tasks-dir",str(task),"--agent","opencode","--model",os.environ.get("PILOT_MODEL",args.model),"--sandbox","docker","--skills-dir",str(skills),"--skill-mode","no-skill" if args.condition=="ZERO" else "with-skill","--usage-tracking","required","--jobs-dir",str(run),"--quiet"]
